from flcore.clients.client_alp_5th_algo_attention import clientalp_v5
from flcore.servers.serverbase import Server
from utils.data_utils import read_client_data
from utils.attention_weights_v2 import EnhancedSelfAttentionLayer
...
class Fed_alp_v5(Server):
    def __init__(self, args, times):
        super().__init__(args, times)

        # select slow clients
        self.set_slow_clients()
        self.set_clients(clientalp_v5)
    ...
    def generate_aggregated_protos(self):
        ...
        input_tensor = torch.zeros(num_clients, num_classes, feature_size).to(self.device)

        # Fill the tensor
        for idx, (client_id, labels_dict) in enumerate(client_label_dict.items()):
            for label, tensor in labels_dict.items():
                input_tensor[idx, label, :] = tensor
        ...
        for epoch in range(25):
            ...
            for client_id,labels_dict in client_label_dict.items():
                ...
                input_tensor_client = torch.zeros(num_clients, num_classes, feature_size)
                for label, tensor in labels_dict.items():
                    for idx in range(num_clients):
                        input_tensor_client[idx, label, :] = tensor

                output, attention_weights,output_proto = model(input_tensor_client,input_tensor)
                final_client_proto_dict[client_id] = output_proto
                loss = F.mse_loss(output, input_tensor_client[0].to(self.device))
                total_loss += loss
        ...
        for client_id,labels_dict in client_label_dict.items():
            ...
            for i in range(self.num_classes):
                client_label_dict_attention[client_id][i] = final_client_proto_dict[client_id][i].detach()
