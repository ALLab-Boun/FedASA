from flcore.clients.client_alp_8th_algo_attention import clientalp_v8
from flcore.servers.serverbase import Server
from utils.data_utils import read_client_data
from utils.attention_weights_v5 import EnhancedSelfAttentionLayer
...
class Fed_alp_v8(Server):
    def __init__(self, args, times):
        super().__init__(args, times)

        # select slow clients
        self.set_slow_clients()
        self.set_clients(clientalp_v8)
        ...
        for epoch in range(50):
            ...
            for client_id,labels_dict in client_label_dict.items():
                ...
                for label_,proto_ in labels_dict.items():
                    ...
                    client_weights, output,resulting_prototype_wa_weights = model(proto_,class_input_tensor_dict[label_])
                    if client_id in final_client_proto_dict:
                        final_client_proto_dict[client_id][label_] = resulting_prototype_wa_weights
                    else:
    ...
    def generate_aggregated_protos_old(self):
            ...
            models = {i: EnhancedSelfAttentionLayer(feature_size, intermediate_size, num_attention_weights).to(self.device) for i in range(num_classes)}
